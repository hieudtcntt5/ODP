...
class AirflowScheduler(Script):
	...
	def install(self, env):
		...
		airflow_flower_make_startup_script(env)
	...
	def configure(self, env):
		...
		airflow_configure(env)
		airflow_flower_make_startup_script(env)
		airflow_make_systemd_scripts_flower(env)
		
	def start(self, env):
		import params
		self.configure(env)
		Execute("service airflow-flower start")
	...
	def stop(self, env):
		...
		Execute("service airflow-flower stop")
		File(params.airflow_flower_pid_file,
			action = "delete",
	...
	def status(self, env):
		...
		env.set_params(params)
		check_process_status(params.airflow_flower_pid_file)
